...
function generateFrame(cmd, addr, data) {
...
function activate(context) {
    let disposable = vscode.commands.registerCommand('plugin-spi-host.flash', async (uri) => {
        ...
        const selectedPort = await vscode.window.showQuickPick(ports.map(p => ({ label: p.path, description: p.manufacturer || "Périphérique série" })), {
            placeHolder: "1. Sélectionnez le port COM de l'interface SPI Host",
            ignoreFocusOut: true
        });
        if (!selectedPort) {
            return;
        } // Annulation si l'utilisateur appuie sur Echap
